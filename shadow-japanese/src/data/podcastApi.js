...
async function fetchTextWithFallback(url) {
  try {
    ...
    if (!proxyResponse.ok) {
      throw new Error(`RSS读取失败：${directError.message}; proxy HTTP ${proxyResponse.status}`);
    }
    return proxyResponse.text();
  }
}

export async function fetchPodcastFeed(source, limit = 10) {
  const xml = await fetchTextWithFallback(source.url);
  return parsePodcastXml(xml, source).slice(0, limit);
}

export async function fetchPodcastSourceById(sourceId, limit = 10) {
  const source = PODCAST_SOURCES.find(item => item.id === sourceId);
  if (!source) throw new Error(`未知播客源：${sourceId}`);
  return fetchPodcastFeed(source, limit);
}
